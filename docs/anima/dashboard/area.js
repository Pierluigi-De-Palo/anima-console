/* AREA PERSONALE — la porta che si apre da soli, con la card in mano.
   ...
   non e' una password scelta da qualcuno, non si indovina.
...
(function () {
  ...
  // Il codice si puo' scrivere come viene: minuscolo, senza trattini, con spazi.
  // Conta solo lettere e cifre. Stessa regola in scripts/build_card_pubblica.py.
  function normalizza(t) { return (t || '').toUpperCase().replace(/[^A-Z0-9]/g, ''); }
    ...
    return sha256('carta|' + dati.card_id + '|' + seriale).then(function (prova) {
      if (prova !== dati.verifica_ingresso) {
        dillo('Il codice non corrisponde a questa carta. Ricontrolla il retro: '
              + 'sono dodici lettere in tre gruppi.');
        return false;
      }
  ...
  function prova() {
    dillo('');
    var quale = normalizza(campoId.value || id);
    var seriale = normalizza(campoCodice.value);
